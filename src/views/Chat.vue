<template>
    <div class="no-bottom no-top" id="content">
        ...
        <section aria-label="section">
            <div class="container chat col-md-12">
                <div class="col-md-3 chat__contacts">
                    <div class="contact_header">
                        <h3>Contacts</h3>
                    </div>
                    <div class="contact_info"
                        :class="{ active : activeChat.contact && (activeChat.contact.ticketId == contact.ticketId) }"
                        v-for="(contact,index) in contacts"
                        :key="index+contact.ticketId"
                        @click="() => { openUserChat( contact.sender._id ) }"
                    >

                        <div class="contact_avatar">
                            <img :src="contact.sender.avatar || defaultImg" alt="">
                        </div>
                ...
                <div class="col-md-9 chat__logs">
                    <div class="autoScroll" ref="scrollToMe">
                        <div
                            v-for="(msgGrp, index) in formattedChatLog"
                            :key="msgGrp.senderId+String(index)"
                            class="chatLog"
                            :class="{'chatLog-left': msgGrp.senderId === activeChat.contact.sender._id}"
                        >
                            <div class="chatLog-avatar">
                                <img :src="msgGrp.senderId === activeChat.contact.id ? activeChat.contact.avatar : defaultImg" alt="">
                            </div>
                            <div class="chatLog-body">
                                <div
                                    v-for="msgData in msgGrp.messages"
                                    :key="msgData.time"
                                    class="chatLog-content"
                                >
                                    <div>
                    ...
                    <div class="sendInput" v-if="activeChat.contact">
                        <input 
                            type="text" 
...
export default {
    ...
    methods: {
        ...
        openUserChat( userId ) {
            this.openChat({ userId }).then(res => {
                this.activeChat = res.data
					
                // Set unseenMsgs to 0
                const contact = this.contacts.find(c => c.sender._id === userId)

                if (contact) contact.unread = 0

                this.activeChat.contact = {...contact};

                this.sortMessages();
                setTimeout(this.scrollToElement, 10);
            });
        },
        sendMessage() {
            ...
            const payload = {
				receiverId: this.activeChat.contact.sender._id,
				message: this.chatInputMessage,
			}

            this.sendChatMessage(payload).then(res => {
                const { chat } = res.data
                this.activeChat.chat = chat;

                // Reset send message input value
                this.chatInputMessage = ''

                // Set Last Message for active contact
                const contact = this.contacts.find(c => c.ticketId === this.activeChat.contact.ticketId)
        ...
        sortMessages() {
            ...
            chatLog.forEach((msg, index) => {
                if (chatMessageSenderId === msg.sender) {
                    ...
                    this.formattedChatLog.push(msgGroup)
                    msgGroup = {
                        senderId: msg.sender,
                        messages: [{
                            msg: msg.content,
                ...
                if (index === chatLog.length - 1) this.formattedChatLog.push(msgGroup)
            })
        },
        scrollToElement() {
